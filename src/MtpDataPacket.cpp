...
int MtpDataPacket::write(struct usb_request *request, UrbPacketDivisionMode divisionMode) {
    ...
    while (processedBytes < mPacketSize) {
        ...
        if (result < 0) {
            LOG(ERROR) << "Failed to write bytes to the device.";
            return -1;
        }
...
int MtpDataPacket::write(struct usb_request *request,
    ...
    if (request->max_packet_size <= 0) {
        LOG(ERROR) << "Cannot determine bulk transfer size due to illegal max packet size " << request->max_packet_size;
        return -1;
    }
    ...
    while (processedBytes < containerLength) {
        ...
        if (headerSent || divisionMode == FIRST_PACKET_HAS_PAYLOAD) {
            ...
            if (!readError) {
                ...
                if (result < 0) {
                    LOG(ERROR) << "Found an error while reading data from FD. Send 0 data instead.";
                    readError = true;
                }
            }
            if (readError) {
        ...
        if (result != static_cast<ssize_t>(bulkTransferSize)) {
            ...
            LOG(ERROR) << "Found an error while write data to MtpDevice.";
            return -1;
        }
